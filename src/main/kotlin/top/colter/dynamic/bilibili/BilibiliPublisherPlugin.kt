...
public class BilibiliPublisherPlugin() :
    ...
    override suspend fun resolveLink(parsedLink: ParsedLink): LinkResolution {
        ...
        val source = runCatching { pollService.fetchDynamicDetail(parsedLink.targetId) }
            .getOrElse { error ->
                return LinkResolution.Failed(
                    parsedLink = parsedLink,
    ...
    private suspend fun resolveVideoPreview(parsedLink: ParsedLink): LinkResolution {
        val snapshot = runCatching { pollService.fetchVideoSnapshot(parsedLink.targetId) }
            .getOrElse { error ->
                return LinkResolution.Failed(
                    parsedLink = parsedLink,
    ...
    private suspend fun resolveLivePreview(parsedLink: ParsedLink): LinkResolution {
        val snapshot = runCatching { pollService.fetchLiveRoomSnapshot(parsedLink.targetId) }
            .getOrElse { error ->
                return LinkResolution.Failed(
                    parsedLink = parsedLink,
        ...
        val publisher = snapshot.userId.takeIf { it.isNotBlank() }?.let { fetchPublisherInfo(it) }
        return LinkResolution.Preview(
            parsedLink = parsedLink,
            preview = LinkPreview(
                platformId = platformId,
    ...
    private suspend fun resolveUserPreview(parsedLink: ParsedLink): LinkResolution {
        val publisher = runCatching { fetchPublisherInfo(parsedLink.targetId) }
            .getOrElse { error ->
                return LinkResolution.Failed(
                    parsedLink = parsedLink,
